  ...
  actions: {
      ...
      let styles = {
        ...
        wsDroneVector: [
          new Style({
            stroke: new Stroke({
              color:
                wsDroneMarker[fplIdentifier]?.get("type") == "adsb2"
                  ? designAircrft.adsbColor
                  : designAircrft.acceptColor,
              width: 1,
            }),
          }),
          new Style({
    ...
    removeWsDroneData: (id: string) => {
      set((state) => {
        ...
        newWsDroneMarker[id]?.remainCorridor &&
          vectorSource.removeFeature(newWsDroneMarker[id].remainCorridor);
        newWsDroneMarker[id] &&
          vectorSource.removeFeature(newWsDroneMarker[id]);
        newWsLabelLine[id] && vectorSource.removeFeature(newWsLabelLine[id]);
        newWsDroneVector[id] &&
          vectorSource.removeFeature(newWsDroneVector[id]);
        ...
        return {
          wsDroneMarker: newWsDroneMarker,
          wsLabelLine: newWsLabelLine,
          wsDroneLabel: newWsDroneLabel,
          wsDroneVector: newWsDroneVector,
        };
      });
    },
    pushWsDroneData: async (payload) => {
      const {
        ...
        wsLabelLine,
        isShowVector,
        actions: { styleFunction, createDroneLabel, updateDroneLabel },
      } = get();
      const { olMap, vectorSource, getLayer, mapRotation } =
        useMapStore.getState();
      const { FPLList } = useFPLStore.getState();
      let flightPlanIdentifier = payload.result.flightPlanIdentifier;
      if (flightPlanIdentifier == undefined)
        flightPlanIdentifier = "test-identifier";
      if (
        payload.result.gps_lat == 0 ||
        payload.result.gps_lon == 0 ||
        payload.result.gps_lat >= 360 ||
        payload.result.gps_lon >= 360
      ) {
        console.log("촬영드론 실시간 모니터링 좌표 오류");
      }

      const newPoint = transform(
        [payload.result.gps_lon, payload.result.gps_lat],
        "EPSG:4326",
        "EPSG:5179"
      );
      const altitudeInRange = applyFilter
        ? filteredAltitude.min &&
          Number(payload.result.altitude) >= filteredAltitude.min &&
          filteredAltitude.max &&
          Number(payload.result.altitude) <= filteredAltitude.max
        : true; // applyFilter가 적용되지 않으면 모든 기체 표시

      //해당 항적에 관련된 FPL정보
      let FPLInfo = FPLList?.filter(
        (ind) => ind.flightPlanIdentifier == flightPlanIdentifier
      );

      // 고도 필터링 (false -> 삭제)
      if (altitudeInRange) {
        if (wsDroneMarker[flightPlanIdentifier] == undefined) {
          //드론 아이콘 없을시
          ...
          newDroneMarker.destinationAerodrome =
            payload.result.destinationAerodrome;
          ...
          if (FPLInfo.length > 0) {
            newDroneMarker.submitterInfo = FPLInfo[0].flightPlanSubmitterInfo
              ? FPLInfo[0].flightPlanSubmitterInfo
              : FPLInfo[0].flightPlanSubmitter; // submitter 정보
            newDroneMarker.cruisingLevel = FPLInfo[0].cruisingLevel;
            newDroneMarker.memo = FPLInfo[0].memo || "";
            newDroneMarker.indicatedAltitude =
              Number(FPLInfo[0].indicatedAltitude?.split(" ")[0]) || 0; // FPL 지시고도
          }
          ...
          if (FPLInfo?.length > 0) {
            updateDroneMarker.submitterInfo = FPLInfo[0].flightPlanSubmitterInfo
              ? FPLInfo[0].flightPlanSubmitterInfo
              : FPLInfo[0].flightPlanSubmitter; // submitter 정보
            updateDroneMarker.memo = FPLInfo[0].memo || "";
            updateDroneMarker.indicatedAltitude =
              Number(FPLInfo[0].indicatedAltitude?.split(" ")[0]) || 0; // FPL 지시고도
          }
          ...
          if (!get().wsDroneLabel[flightPlanIdentifier]?.isDragging) {
            ...
            const pixelLabel = olMap.getPixelFromCoordinate(
              updateDroneLabel.getPosition()
            );
            let xOffset =
              pixelDrone[0] - pixelLabel[0] + updateDroneLabel.get("x");
            let yOffset =
              pixelDrone[1] - pixelLabel[1] + updateDroneLabel.get("y");
            let adjustedCoord = olMap.getCoordinateFromPixel([
              pixelLabel[0] + xOffset,
              pixelLabel[1] + yOffset,
            ]);
            ...
            if (get().wsDroneMarker[flightPlanIdentifier].isTarget == true) {
              ...
              let offsetAngle =
                (Number(updateDroneMarker.heading) + mapRotation + 180) % 360; // 반대 방향의 각도
              ...
              adjustedPixelCoord = [
                pixelDrone[0] + offsetX,
                pixelDrone[1] - offsetY,
              ]; // Y축 방향 반전
            ...
            updateLabelLine
              .getGeometry()
              .setCoordinates([newPoint, adjustedCoord]);
    ...
    createDroneLabel: async (flightPlanIdentifier: string) => {
      ...
      const updateDroneMarker = wsDroneMarker[flightPlanIdentifier];
      let convertAltitude = UtilFunc.convertKm(
        updateDroneMarker.altitude * 0.0003048
      ); // 단위:FT, 1FT == 0.0003048 KM
      let convertIndicatedAlt = UtilFunc.convertKm(
        updateDroneMarker.indicatedAltitude * 0.0003048
      ); // 단위:FT, 1FT == 0.0003048 KM
      let getAltitudeUnit = altitudeUnit == null ? "FT" : altitudeUnit; // 지시고도 단위

      content.innerHTML = `
          <div class="label-row1">
            <span class="vertiport">${
              updateDroneMarker.destinationAerodrome
            }</span>
            <!--<span class="landingDirection"></span>-->
            <span class="warning" style="${
              updateDroneMarker?.warnColor
                ? `color: ${wsDroneMarker[flightPlanIdentifier].warnColor};`
                : ""
            }">${wsDroneMarker[flightPlanIdentifier]?.warnType || ""}</span>
          </div>
          <div class="label-row2">
            <span class="aircraft-info">${updateDroneMarker.callsign} ${
        updateDroneMarker.aircraftType
      }</span>
          </div>
          <div class="label-row3">
            <span class="altitude">${Math.floor(
              convertAltitude[getAltitudeUnit]
            )}</span>
            <span class="nowUpDown" style="color: yellow; width: 5%;"></span>
            <span id="indicated-alt-${flightPlanIdentifier}" style="display: inline-block; text-align: center;">${Math.floor(
        convertIndicatedAlt[getAltitudeUnit]
      )}</span>
            <span class="horVelocityKTS">${
              updateDroneMarker.horVelocityKTS
            }</span>
          </div>
          <div class="memo-section">
            <div class="memo-content" id="memo-${flightPlanIdentifier}">${
        updateDroneMarker.memo || ""
      }</div>
          </div>
        `;
      ...
      if (savedDataJSON) {
        ...
        if (positioning === "target") {
          ...
          adjustedPixelCoord = [
            pixelCoord[0] + offset[0],
            pixelCoord[1] + offset[1],
          ];
          adjustedCoord = olMap.getCoordinateFromPixel(adjustedPixelCoord);
        }
      } else {
        ...
        localStorage.setItem(
          "labelPosition",
          JSON.stringify({ positioning: positioning, offset: offset })
        );
      }
    ...
    updateDroneLabel: (flightPlanIdentifier: string) => {
      ...
      let altElement = wsDroneLabel[flightPlanIdentifier]
        .getElement()
        .querySelector(".altitude"); // 현재고도
      let indicatedAltElement = wsDroneLabel[flightPlanIdentifier]
        .getElement()
        .querySelector(`#indicated-alt-${flightPlanIdentifier}`); // 지시고도

      let nowUpDownElement = wsDroneLabel[flightPlanIdentifier]
        .getElement()
        .querySelector(".nowUpDown"); // 상승하강 표시 업데이트
      let horVelocityKTSElement = wsDroneLabel[flightPlanIdentifier]
        .getElement()
        .querySelector(".horVelocityKTS"); // Ground-Speed 업데이트
      let landingDirectionElement = wsDroneLabel[flightPlanIdentifier]
        .getElement()
        .querySelector(".landingDirection"); // 버티포트 착륙방향 업데이트(추후)

      if (altElement && nowUpDownElement) {
        let prevAltitude = Math.floor(
          wsDroneMarker[flightPlanIdentifier].previousAltitude
        );
        let nowAltitude = Math.floor(
          wsDroneMarker[flightPlanIdentifier].altitude
        );

        let convertNowAlt = UtilFunc.convertKm(nowAltitude * 0.0003048); // 단위:FT, 1FT == 0.0003048 KM
        let convertIndicatedAlt = UtilFunc.convertKm(
          wsDroneMarker[flightPlanIdentifier].indicatedAltitude * 0.0003048
        );
        ...
        // 지시고도 업데이트
        indicatedAltElement.innerText = Math.floor(
          convertIndicatedAlt[getAltitudeUnit]
        );

        let updown =
          nowAltitude > prevAltitude
            ? "↑"
            : nowAltitude < prevAltitude
            ? "↓"
            : "";
      ...
      if (horVelocityKTSElement) {
        horVelocityKTSElement.innerText = Math.floor(
          wsDroneMarker[flightPlanIdentifier].horVelocityKTS
        );
      }
    },
